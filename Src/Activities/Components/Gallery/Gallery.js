...
export default class Gallery extends React.Component {
    constructor(props) {
        super(props);
        this.imageWidth = (width - (2 * (props.galleryMargin + props.galleryPadding))) / props.numColumns
        this.state = {
            items: [],
            selectedImages: [],
            after: "",
    ...
    componentDidMount() {
        Albums.getAlbumList({
            ...
            const dataItems = list.map((item, i) => {
                return <Item key={i + 1} label={item.name} value={item.name}/>;
            });
            this.setState({
    ...
    onButtonPress = itemData => {
        ...
        this.setState({selectedImages, items: newItems})
    ...
    renderFlatListItem = itemData => {
        ...
        return (
            <TouchableWithoutFeedback
                onPress={() => this.onButtonPress(itemData)}
            >
